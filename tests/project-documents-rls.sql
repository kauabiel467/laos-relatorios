...
insert into public.agency_documents(client_id,kind,title,config,data,status,created_by) values
 ('aecc0000-0000-4000-8000-000000000001','dashboard','Draft dashboard','{}','{}','draft','aeaa0000-0000-4000-8000-000000000001'),
 ('aecc0000-0000-4000-8000-000000000001','report','Published report','{}','{}','published','aeaa0000-0000-4000-8000-000000000001'),
 ('aecc0000-0000-4000-8000-000000000001','template','Internal template','{}',null,'draft','aeaa0000-0000-4000-8000-000000000001');
set local role authenticated;
select set_config('request.jwt.claim.sub','aeaa0000-0000-4000-8000-000000000002',true);
do $$ begin
 if (select count(*) from public.agency_documents where client_id='aecc0000-0000-4000-8000-000000000001')<>1 then raise exception 'Client document visibility failed';end if;
 update public.agency_documents set title='Unauthorized' where client_id='aecc0000-0000-4000-8000-000000000001';if found then raise exception 'Client document edit allowed';end if;
 begin perform 1 from public.agency_meta_connections;raise exception 'Credentials exposed';exception when insufficient_privilege then null;end;
...
do $$ begin
 if exists(select 1 from public.agency_documents where client_id='aecc0000-0000-4000-8000-000000000001') then raise exception 'Document crossed client boundary';end if;
...
do $$ begin
 if (select count(*) from public.agency_documents where client_id='aecc0000-0000-4000-8000-000000000001')<>3 then raise exception 'Staff documents hidden';end if;
 update public.agency_documents set title='Edited dashboard' where client_id='aecc0000-0000-4000-8000-000000000001' and kind='dashboard';if not found then raise exception 'Dashboard update failed';end if;
 begin update public.agency_documents set title='Edited publication' where client_id='aecc0000-0000-4000-8000-000000000001' and kind='report';raise exception 'Report mutation allowed';exception when raise_exception then if sqlerrm='Report mutation allowed' then raise;end if;end;
 delete from public.agency_documents where client_id='aecc0000-0000-4000-8000-000000000001' and status='published';if found then raise exception 'Published deletion allowed';end if;
end $$;
set local role anon;
do $$ begin
...
rollback;
select 'PASS: project documents, templates, credentials isolation, publication immutability, client visibility, no client writes, outsider isolation, no ownership takeover, atomic teams, staff drafts, immutable publication, anonymous isolation; fixtures rolled back' as result;
