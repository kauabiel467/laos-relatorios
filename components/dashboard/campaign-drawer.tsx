...
export function CampaignDrawer({ campaign, ads, onClose }: CampaignDrawerProps) {
        ...
        <div className="space-y-3">
          {ads.map((ad: AdItem) => (
            <div key={ad.id} className="rounded-xl border border-border bg-bg p-4">
              ...
              <div className="mb-2 text-sm font-semibold">{ad.name}</div>
              <div className="flex flex-wrap gap-4 font-mono text-[11px] text-muted">
                ...
                <span>Spend <strong className="font-medium text-text">{formatCurrency(ad.spend)}</strong></span>
              </div>
            </div>
          ))}
        </div>
